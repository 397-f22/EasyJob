...
describe("Delete job", () => {
    it("should call removeJob function once", async () => {
      ...
      await render(<Subheader jobs={mockUser1["jobs"]} user={mockUser1} />, {wrapper: BrowserRouter});

      // check data is correctly displayed
      expect(await screen.getByTestId("Applied List").textContent).toContain('Niantic');
      expect(await screen.getByTestId("Online Assessment List").textContent).toBe('No jobs with this status');
      expect(await screen.getByTestId("Interview List").textContent).toBe('No jobs with this status');
      expect(await screen.getByTestId("Offered List").textContent).toContain('The Pokemon Company');
      expect(await screen.getByTestId("Their Loss List").textContent).toBe('No jobs with this status');
    ...
    it("should not be in any filtered job lists"), async () => {
      getUser.mockReturnValue([mockUser2]);
      await render(<Subheader jobs={mockUser2["jobs"]} user={mockUser2} />, {wrapper: BrowserRouter});

      // check data is correctly displayed
      expect(await screen.getByTestId("Applied List").textContent).toContain('Niantic');
      expect(await screen.getByTestId("Online Assessment List").textContent).toBe('No jobs with this status');
      expect(await screen.getByTestId("Interview List").textContent).toBe('No jobs with this status');
      expect(await screen.getByTestId("Offered List").textContent).not.toContain('The Pokemon Company');
      expect(await screen.getByTestId("Their Loss List").textContent).toBe('No jobs with this status');
    }
})